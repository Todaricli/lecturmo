...
export async function createEmptyCourses() {
  const courses = await Promise.all(Array.from({ length: NUMBER_OF_COURSES }, () => new Course().save()));
  return courses
}
...
export async function populateCourses(users, courses) {
  try {
    const updatedCourses = await Promise.all(courses.map(async (course) => {
      ...
      course.reviews = Array.from({ length: faker.number.int({ min: 1, max: 5 }) }, () => ({
        userId: faker.helpers.arrayElement(users)._id, // Random user ID from the fetched users
        content: faker.lorem.sentences(),
        likes: users.filter(() => faker.datatype.boolean()).map(user => user._id),
