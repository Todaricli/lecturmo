...
router.get('/summarizeReview', async (req, res) => {
    ...
    if(!courseDummyId) return res.status(400).json("Please provided a course you want to get summary");

    try {
        const course = await Course.findOne({ course_dummy_id: courseDummyId });

        if (!course) return res.status(500).json("Unable to retrieve course reviews data")

        const courseName = JSON.stringify(course.course_dummy_id + ': ' + course.courseName)
        const reviewArray = JSON.stringify(course.reviews);

        if (!reviewArray) return res.status(400).json("This course has no review yet!")
